...
The model can be shown in the Figure~\ref[img_tx2_auto_3]. The further descriptions of hardware stack is specific to
this model.
...
* Teensy MCU – An auxiliary microcontroller for controlling the steering and communicating with an RC Transmitter
(Manual Control).
...
(which is done in Chapter~\ref[follow_the_gap]), but also of setting up the NVIDIA Jetson TX2 so it can run ROS 2
...
in the Section 4.3.2 in~\cite[klapalek_avoidance_2019].

Because migrating all the code of CTU F1/10 platform at once would not be feasible, we need to select a some part of
it that we will actually try to migrate. Such part should meet at least the following criteria:
\begitems
* It is possible to easily demonstrate its working in a simulator and or the real model.
...
All these criteria all met by the {\em Follow the Gap} application which is a part the CTU F1/10 platform.
It consists implements the Follow the Gap algorithm that was introduced in ~\cite[paper_follow_the_gap].
